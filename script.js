// Global Supabase Configuration (runs immediately)
...
// GLOBAL FUNCTIONS - These must be defined at top level for onclick to work
...
function registerPlayer() {
  ...
  addPlayerToSupabase(name, amount);
}

// Add Contribution (GLOBAL)
function addContribution() {
  ...
  addContributionToSupabase(name, amount);
}

// Copy M-Pesa Number (GLOBAL)
function copyMpesaNumber() {
  console.log('📋 Copying M-Pesa number...');
...
// SUPABASE FUNCTIONS - These run after DOM loads
document.addEventListener('DOMContentLoaded', function() {
  ...
  console.log('🚀 Irindiro FC app fully ready!');
});

// Helper function to add player to Supabase
async function addPlayerToSupabase(name, amount) {
  try {
...
// Helper function to add contribution to Supabase
async function addContributionToSupabase(name, amount) {
  try {
    const { error } = await supabase
      .from('contributions')
      .insert([{ name, amount }]);
